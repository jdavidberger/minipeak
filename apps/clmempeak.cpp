...
__kernel void memread(__global const float *inptr, __global float *ptr, uint access_stride)
{
...
#define CL_MEM_FLAG(x) { #x, x }
	...
	auto flags = buffer.b->getInfo<CL_MEM_FLAGS>();
        ...
        for(auto& m : memaccess) {
            ...
            auto kernel = OCL::Kernel("memread", {buffer.cl_constants(), source }, {buffer, out_buffer});
